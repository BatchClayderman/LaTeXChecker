...
class Checker:
					...
					if "{" in line and "," in line:
						...
						if citation in self.__citations:
							if self.__citations[citation][0] is None:
								self.__citations[citation][0] = True
							elif isinstance(self.__citations[citation], list):
								self.__citations[citation][0].append(True)
							else:
								self.__citations[citation][0] = [self.__citations[citation][0], True]
						else:
							self.__citations[citation] = [True, 0] # [isBibTeX, citeCount]
					else:
								...
								if "\\bibitem" == buffer:
									if label in self.__citations:
										if self.__citations[label][0] is None:
											self.__citations[label][0] = False
										...
										self.__citations[label] = [False, 0] # [isBibTeX, citeCount]
										self.__print("A new citation \"{0}\" is added at {1}. ".format(self.__convertEscaped(label), self.__pointer.getCurrentLocationDescription()), Debug)
								elif "\\label" == buffer:
									...
									while True:
										if self.__pointer.hasNextChar(fileSwitch = False):
											...
											if "0" <= ch <= "9":
												...
												self.__print("Only digits instead of the character \"{0}\" can be used to indicate the option count at {1}. ".format("\\" + ch if ch in ("\\", "\"", "\'") else ch, self.__pointer.getCurrentLocationDescription()), Error)
												return False
	...
	def __printStatistics(self:object) -> bool:
		...
		# Citations #
		repeatedCitations, undefinedCitations, uncitedCitations = [], [], []
		...
		if length >= 2:
			self.__print("There are {0} repeated citations found, which are listed as follows. ".format(length), Warning)
			self.__print(repeatedCitations, Warning, indentationCount = 1)
		elif 1 == length:
			self.__print("There is a repeated citation found: \"{0}\". ".format(self.__convertEscaped(repeatedCitations[0])), Warning)
